...
def test_client_credentials_provider_requests_and_caches_token() -> None:
    ...
    provider = ShopifyClientCredentialsTokenProvider(
        _credentials(),
        transport=transport,
        now=lambda: datetime(2026, 1, 1, tzinfo=timezone.utc),  # noqa: UP017
    )
    ...
    assert provider.expires_at == datetime(2026, 1, 1, tzinfo=timezone.utc) + timedelta(  # noqa: UP017
        seconds=3600
    )


def test_client_credentials_provider_refreshes_inside_expiry_buffer() -> None:
    current_time = datetime(2026, 1, 1, tzinfo=timezone.utc)  # noqa: UP017
    transport = FakeTokenTransport(
        [
