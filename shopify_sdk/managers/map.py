...
from typing import Type, Optional, get_args, get_origin, get_type_hints
...
class MapManager(BaseModel):
    ...
    def get(
        ...
        query: Query = self._get_query_class(model)
        if not query:
        ...
        connection_instance = self._build_and_execute_query(
            query, model, field_key, value_key, inclusion_overrides
        )
    ...
    def _build_and_execute_query(
        ...
        if not inclusion_overrides:
            field_inclusions = {
                model.__name__: {field_key, value_key}
            }
        else:
            field_inclusions = inclusion_overrides
        built_query = query(
            field_inclusions=field_inclusions
        )
    ...
    def _get_query_class(
        ...
        :rtype: Type[Query]
        """
        connection_subclasses = connection.__subclasses__() # Get all connection subclasses
        ...
        for conn_class in connection_subclasses:
            ...
            if node_type and issubclass(node_type, model):
                for query_class in query_subclasses: 
                    if query_class.return_type and query_class.return_type == conn_class:
                        return query_class
        return None
