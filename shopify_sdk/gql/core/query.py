...
class Query:
    ...
    def _build_model_selection(self, model: Type[BaseModel], indent: int) -> str:
        ...
        for name in field_names:
            if type_hints.get(name) is None or not self._should_include_field(
                model, name
            ):
                continue
    ...
    def _build_connection_selection(
        ...
        if self._is_model(nodes_type) and self._should_include_field(
            ...
            sections.append(
                f"{' ' * inner_indent}nodes {{\n{nodes_body}\n{' ' * inner_indent}}}"
            )

        page_info_type = self._unwrap_annotation(conn_hints.get("pageInfo"))
        if self._is_model(page_info_type):
            page_info_body = self._build_model_selection(
                page_info_type, inner_indent + self._indent
            )
            sections.append(
    ...
    def _format_connection_args(
        ...
        formatted = ", ".join(
            f"{key}: {self._format_literal(value)}" for key, value in args.items()
        )
        return f"({formatted})"
    ...
    def _format_literal(self, value: Any) -> str:
        ...
        if isinstance(value, (int, float)):
            if isinstance(value, float) and not math.isfinite(value):
                raise ValueError(
                    f"GraphQL numeric literal must be finite, got {value!r}"
                )
            return str(value)
        if value is None:
            return "null"
        ...
        if isinstance(value, dict):
            ...
            for k, v in value.items():
                if not isinstance(k, str):
                    raise TypeError(
                        f"GraphQL object key must be a string, got {type(k).__name__}"
                    )
        ...
        if len(snippet) > 200:
            snippet = snippet[:197] + "..."
        raise TypeError(
            f"Unsupported GraphQL literal {type(value).__name__}: {snippet}"
        )
    ...
    def _build_partial_value(self, value: Any, annotation: Any) -> Any:
        ...
        if origin is list or origin is List:
            ...
            if self._is_model(inner) or self._is_connection(inner):
                ...
                return [
                    self._build_partial_model(item, inner_model)
    ...
    def _graphql_type_for_argument(
        self, name: str, annotation: Any, value: Any
    ) -> Tuple[str, bool]:
        ...
        if resolved is not None:
            ...
            if origin is Union:
                union_args = [
                    arg for arg in get_args(resolved) if arg is not type(None)
                ]
            ...
            if origin is list or origin is List:
                ...
                inner_type, inner_nullable = self._graphql_type_for_argument(
                    name, inner_ann, None
                )
    ...
    def inline_body(self) -> str:
        ...
        args_list = self._input_arguments
        args_inline = ", ".join(
            f"{name}: {self._format_literal(value)}"
    ...
    def execute(self, client: ShopifyClient):
        ...
        if not isinstance(order_data, dict):
            raise TypeError(
                f"Response data for the class must be a dictionary, got {type(order_data).__name__}."
            )
        if self.return_type is None:
    ...
    def bulk(self) -> BaseModel:
        ...
        return return_type(edges=[], nodes=objects, pageInfo=self._fake_page_info())
