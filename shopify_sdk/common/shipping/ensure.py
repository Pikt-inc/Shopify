...
def ensure_order_gid(order_id: ID) -> ID:
    if order_id is None:
...
def ensure_fulfillment_order_line_item_gid(line_item_id: ID) -> ID:
...
def ensure_fulfillment_gid(fulfillment_id: String | Int | ID) -> ID | None:
...
def ensure_line_item_gid(line_item_id: ID) -> ID:
    if line_item_id is None:
        raise ValueError("line_item_id cannot be None")
    string_line_item_id = str(line_item_id)
    if "gid://" in string_line_item_id:
