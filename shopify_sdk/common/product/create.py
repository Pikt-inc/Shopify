...
from shopify_sdk.gql import productCreate, productUpdate
...
def create_product(
    ...
    result: dict[str, Any] = productCreate(product=input).execute(client=client)
...
def update_product(
    ...
    result: dict[str, Any] = productUpdate(product=input).execute(client=client)
    product_id = result.get("product", {}).get("id", None)
    if not isinstance(product_id, str):
        raise ValueError("Product creation failed; no product ID returned.")
    return product_id
